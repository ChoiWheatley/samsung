...
template <class Pred_t> set<char> case_generator(Pred_t predicate) {
  set<char> ret{};
...
template <class Consumer_t> void foreach_state(Consumer_t consumer) {
  for (char state = 0b0000; state <= 0b1111; state++) {
    consumer.operator()(state);
  }
}

class wrong_char_exception {};

enum class ID : char { D = 0b0001, C = 0b0010, B = 0b0100, A = 0b1000 };

inline auto to_char_vec(const string &str) -> vector<char> {
  vector<char> ret{};
  for (char c : str) {
    switch (c) {
    case 'A':
...
namespace sol3 {
/**
 * 동아리활동은 적어도 한 명의 인원이 참여해야 하며, 열쇠를 자신이 들고 가거나
 * 다른 부원에게 넘겨주어야 한다. => 전날에 참여한 인원 중 적어도 한 명은
 * 참석해야 한다.
 *
...
auto solution(const vector<char> &wardens) -> uint64_t {
  ...
  char firstday_warden = wardens.at(0) | static_cast<char>(ID::A);

  // 초기 상태
  foreach_state([&](char state) {
    if (((state & firstday_warden) == firstday_warden)) {
      memo[0][state] = 1;
    }
  });
  ...
  for (size_t day = 1; day < wardens.size(); ++day) {
    ...
    foreach_state([&](char cur_state) {
      if ((cur_state & today_warden) == today_warden) {
        uint64_t sum = 0UL;

        foreach_state([&](char pre_state) {
          if ((pre_state & cur_state) != 0) {
            sum = (sum + memo[day - 1][pre_state]) % MOD;
          }
        });
 ...
 */
inline auto solution(const string &str) -> uint64_t {
 ...
 *
 * 문제에서 추가된 조건에 따라서 ${MOD} 만큼 나눈 나머지를 리턴하라고 했다. 이때
 * 모듈러 덧셈연산을 활용할 수 있다. (A + B) mod C = ((A mod C) + (B mod C)) mod
 * C
 */
...
auto solution(const vector<char> &bits) -> uint64_t {
  ...
  auto pred = [first_day](char a) {
    return ((a & first_day) ==
            first_day); // A와 당번은 첫날 무조건 등교해야 한다.
  };
  uint64_t count = 0UL;
  set<char> possible_cases = case_generator(pred);
